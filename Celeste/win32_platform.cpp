...
#define NOMINMAX
#include <windows.h>
#include "ThirdParty/glcorearb.h" // Include before wglext.h
...
bool platform_create_window(int width, int height, const char * title)
{
    ...
    // Fake Window initializing OpenGL
    {
        ...
        if (!fakeRC)
        {
            ASSERT(false, "Failed to create Render context");
            return false;
        }

        if (!wglMakeCurrent(fakeDC, fakeRC))
        {
            ASSERT(false, "Failed to make current");
            return false;
        }
    ...
    // Actual OpenGL initialization
    {
        ...
        if (!wglChoosePixelFormatARB(dc, pixelAttribs,
            ...
            &numPixelFormats))
        {
            ASSERT(0, "Failed to wglChoosePixelFormatARB");
            return false;
        }
        ...
        {
            ASSERT(0, "Failed to SetPixelFormat");
            return true;
        }
        ...
        if (!rc)
        {
            ASSERT(0, "Failed to crate Render Context for OpenGL");
            return false;
        }

        if (!wglMakeCurrent(dc, rc))
        {
            ASSERT(0, "Faield to wglMakeCurrent");
            return false;
        }
    }
...
void * platform_load_gl_function(const char * funName)
{
    ...
    if (!proc)
    {
        ...
        if (!proc)
        {
            ASSERT(false, "Failed to load gl function %s", "glCreateProgram");
            return nullptr;
        }
    }
